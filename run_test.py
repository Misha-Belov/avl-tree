...
ans_path = (test_path.parent / (f"{test_name}")).with_suffix(".ans")
log_path = Path(f"{test_name}").with_suffix(".log")
...
with (
    ...
    if res.returncode != 0:
        ...
        reslines = list(map(str.strip, res.stdout.splitlines(True)))
        anslines = list(map(str.strip, ans_data.readlines()))
